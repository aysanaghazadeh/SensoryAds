...
def process_files(
    ...
    results_path = os.path.join(args.result_path, 
                                ...
                                '_'.join([
                                        ...
                                        args.MLLM_prompt if args.model_type == 'MLLM' else f'{args.LLM_prompt}',
                                    ]).replace('.jinja', f'_{args.MLLM}{f"_{args.MLLM_prompt}" if args.MLLM_prompt is not None and args.model_type=="LLM" else ""}.json')
                            ).replace('.jinja', '')
