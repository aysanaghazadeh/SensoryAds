...
def _sanitize_q1_list(q1):
    ...
    for p in parts:
        ...
        if not item:
            continue
        lowered = item.lower()
        # Remove only text-like items; keep non-text logos if the model lists them.
        if any(k in lowered for k in ['text', 'wordmark', 'letters', 'numbers', 'caption', 'slogan']):
            continue
...
def _sanitize_image_descriptor_output(text):
    ...
    Keeps Q1/Q2 format but redacts leaked OCR and removes text-like items from Q1.
    """
